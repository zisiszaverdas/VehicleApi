...
public class CategoryReportServiceTests
{
    ...
    {
        _dataStore.Categories.Returns(new ConcurrentBag<Category> { new Category { CategoryId = 1, SpeedLimitKm = 50, SpeedLimitDurationSeconds = 10 } });
        _dataStore.Vehicles.Returns(new ConcurrentBag<Vehicle> { new Vehicle { VehicleId = 1, CategoryId = 1 } });
    ...
    {
        _dataStore.Categories.Returns(new ConcurrentBag<Category> { new Category { CategoryId = 1 } });
        _dataStore.Vehicles.Returns(new ConcurrentBag<Vehicle> { new Vehicle { VehicleId = 1, CategoryId = 1 } });
        _dataStore.Events.Returns(new ConcurrentBag<Event>());
